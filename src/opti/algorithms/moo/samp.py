...
class MORankingCrowdingSAMP(MORankingCrowdingAlgo):
    ...
    def initialize(self, filedir):
        ...
        for k in range(self.n):
            self.sub_populations[k] = self.populationHandler.get_sorted(self.sub_populations[k],
                                                                        self.problemHandler)
    ...
    def step(self):
        for i in range(self.n):
            ...
            self.sub_populations[i] = self.populationHandler.update([self.sub_populations[i],
                                                                temp_population],
                                                                self.problemHandler)
        if self.n > 1:
        ...
        self.population = self.populationHandler.get_sorted(self.population,
                                                            self.problemHandler)
        new_indicator = performance_metrics(self.problemHandler.problem,
                                        self.populationHandler.get_refined(self.population))["HV"]
        ...
        if new_n != self.n:
            ...
            for j in range(self.n):
                self.sub_populations[j] = self.populationHandler.get_sorted(self.sub_populations[j],
                                                                            self.problemHandler)
